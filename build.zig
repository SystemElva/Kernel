...
const imageBuilder = @import("dependencies/image-builder/main.zig");
...
pub fn build(b: *std.Build) void {
    ...
    switch (target_arch) {
        .aarch64 => install_bootloader_step.dependOn(&b.addInstallFile(b.path("dependencies/limine/BOOTAA64.EFI"), "disk/EFI/BOOT/BOOTAA64.EFI").step),
        .x86_64 => install_bootloader_step.dependOn(&b.addInstallFile(b.path("dependencies/limine/BOOTX64.EFI"), "disk/EFI/BOOT/BOOTX64.EFI").step),
        else => unreachable
    }
    install_bootloader_step.dependOn(&b.addInstallFile(b.path("dependencies/limine/config.txt"), "disk/boot/limine/limine.conf").step);
    ...
    switch (target_arch) {
        .aarch64 => {
            ...
            // for UEFI emulation
            qemu_args.appendSlice(&.{"-bios", "dependencies/debug/aarch64_OVMF.fd"}) catch @panic("OOM");
        ...
        .x86_64 => {
            ...
            // for UEFI emulation
            qemu_args.appendSlice(&.{"-bios", "dependencies/debug/x86_64_OVMF.fd"}) catch @panic("OOM");
        },
        else => unreachable
    }
